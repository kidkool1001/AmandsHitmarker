...
public class AmandsHitmarkerClass : MonoBehaviour
{
    ...
    public static DamageInfoStruct damageInfo = new();
    ...
    public static DamageInfoStruct killDamageInfo = new();
    ...
    public static List<AmandsAnimatedImage> amandsAnimatedImages = [];
    ...
    public static List<int> Combo = [];

    public static GameObject ActiveUIScreen;
    public static Dictionary<string, Sprite> LoadedSprites = new();
    public static Dictionary<string, Sprite> LoadedRanks = new();
    public static Dictionary<string, AudioClip> LoadedAudioClips = new();
    ...
    private static Vector3 TLHOffset = new(-1, 1, 0);
    private static Vector3 TRHOffset = new(1, 1, 0);
    private static Vector3 BLHOffset = new(-1, -1, 0);
    private static Vector3 BRHOffset = new(1, -1, 0);
    ...
    public static List<string> DebugWeapons =
        ["RD-704", "MGSL", "P90", "MCX .300 BLK", "G36 E", "FN 5-7", "AXMC", "SV-98"];
    public static List<string> DebugNames =
    [
        "Mellone", "Pin", "Big Pipe", "Birdeye", "Glukhar", "Killa", "Knight", "Reshala", "Sanitar", "Shturman",
        "Tagilla", "Zryachiy"
    ];

    private static AnimationCurve animationCurve = new();
    ...
    private static AnimationCurve AlphaAnimationCurve = new();
    ...
    public static void XPFormula()
    {
        ...
        Combo = [0, 20, 30, 40, 50, 60, 70, 80, 90, 100];
    }
    ...
    public void Update()
    {
        if ((hitmarker || killHitmarker) && ActiveUIScreen != null)
        {
            ...
            if (!AHitmarkerPlugin.EnableBleeding.Value && 
                tmpKillHitmarker && 
                !tmpHitmarker) 
                return;

            if (AHitmarkerPlugin.EnableSounds.Value && 
                !DebugMode)
            {
                if (LoadedAudioClips.TryGetValue(AHitmarkerPlugin.HitmarkerSound.Value, out var clip))
                    audioClip = clip;
            }
            HitmarkerColor = AHitmarkerPlugin.HitmarkerColor.Value;
            if (bodyPart == EBodyPart.Head)
            {
                if (LoadedSprites.TryGetValue(AHitmarkerPlugin.HeadshotShape.Value, out var loadedSprite))
                    sprite = loadedSprite;
            ...
            {
                if (LoadedSprites.TryGetValue(AHitmarkerPlugin.Shape.Value, out var loadedSprite))
                    sprite = loadedSprite;
            }
            if (tmpHitmarker)
            {
                ...
                !tmpKillHitmarker)
            {
                ...
                if (tmpArmorBreak && 
                    AHitmarkerPlugin.EnableSounds.Value && 
                    !DebugMode)
                    audioClip = LoadedAudioClips[AHitmarkerPlugin.ArmorBreakSound.Value];
                else if (AHitmarkerPlugin.EnableSounds.Value && 
                         !DebugMode)
        ...
        if (UpdateDamageNumber)
        {
            ...
            if (damageNumberTextMeshPro != null) 
                damageNumberTextMeshPro.alpha = damageNumberOpacity;
    ...
    public static void Killfeed()
    {
        ...
        if (AHitmarkerPlugin.KillUpperText.Value)
        {
            ...
            switch (killLethalDamageType)
            {
                ...
                case EDamageType.HeavyBleeding:
                    UpperText = "<color=#" + 
                                ColorUtility.ToHtmlStringRGB(AHitmarkerPlugin.BleedColor.Value) + 
                                ">" + 
                                "BLEEDING" + 
                                "</color> ";
                ...
                case EDamageType.Poison:
                    UpperText = "<color=#" + 
                                ColorUtility.ToHtmlStringRGB(AHitmarkerPlugin.PoisonColor.Value) + 
                                ">" + 
                                "POISON" + 
                                "</color> ";
    ...
    {
        if (!AHitmarkerPlugin.EnableMultiKillfeed.Value) 
            return;
        ...
        if (killBodyPart == EBodyPart.Head && 
            AHitmarkerPlugin.MultiKillfeedColorMode.Value != EMultiKillfeedColorMode.SingleColor)
            ...
            case EPlayerSide.Bear:
                switch (AHitmarkerPlugin.MultiKillfeedPMCIconMode.Value)
                {
                    ...
                    case EMultiKillfeedPMCMode.Ranks:
                        int num = (killLevel > 80) ? 
                            80 : (((int)((float)killLevel / 5f) + 1) * 5);
                            
                        if (LoadedRanks.ContainsKey("Rank" + num + ".png")) 
                            sprite = LoadedRanks["Rank" + num + ".png"];
    ...
    {
        if (!AHitmarkerPlugin.EnableKillfeed.Value) 
            return;
    ...
    public static void CreateKillText()
    {
        ...
        if (killPlayerSide == EPlayerSide.Savage)
        {
            ...
            switch (RoleName)
            {
                ...
                case "RAIDER":
                    ...
                    break;
                default:
                    if (AmandsHitmarkerHelper.IsFollower(killRole)) 
                        RoleColor = AHitmarkerPlugin.FollowerColor.Value;
                        
                    if (AmandsHitmarkerHelper.CountAsBossForStatistics(killRole)) 
                        RoleColor = AHitmarkerPlugin.BossColor.Value;
        ...
        switch (AHitmarkerPlugin.KillStart.Value)
        {
            case EKillStart.PlayerWeapon:
                ...
                Start = "<b><color=#" + 
                        ColorUtility.ToHtmlStringRGB(playerRoleColor) + 
                        ">" + 
                        aggNickname + 
                        "</color> " + 
                        killWeaponName + 
            ...
            case EKillStart.WeaponRole:
                Start = "<b>" + 
                        killWeaponName + 
                        " <color=#" + 
                        ColorUtility.ToHtmlStringRGB(RoleColor) + 
                        ">" + 
                        RoleName + 
                        "</color></b> ";
        ...
        if (killDistance > AHitmarkerPlugin.KillDistanceThreshold.Value && 
            !(AHitmarkerPlugin.KillEnd.Value == EKillEnd.Distance || AHitmarkerPlugin.KillEnd.Value == EKillEnd.None))
        {
        ...
        else
        {
            ...
            if (killEnd == EKillEnd.Level && 
                killPlayerSide == EPlayerSide.Savage) 
                killEnd = EKillEnd.Experience;
            ...
            switch (killEnd)
            {
                ...
                case EKillEnd.Experience:
                    ...
                    if (killBodyPart == EBodyPart.Head && 
                        AHitmarkerPlugin.KillHeadshotXP.Value == EHeadshotXP.OnFormula)
                    ...
                    End = "<b>" + 
                          (int)(BaseExp + HeadshotExp + StreakExp) + 
                          "XP</b>";
    ...
    public static void RaidKillfeed(EPlayerSide aggressorSide, WildSpawnType aggressorRole, string aggressorNickname, string weaponName, EDamageType lethalDamageType, EPlayerSide victimSide, WildSpawnType victimRole, string victimNickname)
    {
        ...
        if (aggressorSide == EPlayerSide.Savage)
        {
            ...
            switch (aggressorRoleName)
            {
                ...
                case "RAIDER":
                    ...
                    break;
                default:
                    if (AmandsHitmarkerHelper.IsFollower(aggressorRole)) 
                        aggressorColor = AHitmarkerPlugin.FollowerColor.Value;
                        
                    if (AmandsHitmarkerHelper.CountAsBossForStatistics(aggressorRole)) 
                        aggressorColor = AHitmarkerPlugin.BossColor.Value;
                    break;
            }
        }
        ...
        if (victimSide == EPlayerSide.Savage)
        {
            ...
            switch (victimRoleName)
            {
                ...
                case "RAIDER":
                    ...
                    break;
                default:
                    if (AmandsHitmarkerHelper.IsFollower(victimRole)) 
                        victimColor = AHitmarkerPlugin.FollowerColor.Value;
                        
                    if (AmandsHitmarkerHelper.CountAsBossForStatistics(victimRole)) 
                        victimColor = AHitmarkerPlugin.BossColor.Value;
        ...
        {
            Start = "<b><color=#" + 
                    ColorUtility.ToHtmlStringRGB(aggressorColor) + 
                    ">" + 
                    aggressorRoleName + 
                    "</b> " + 
                    aggressorNickname + 
            ...
            if (AHitmarkerPlugin.RaidKillNameColor.Value == EKillNameColor.Colored && 
                (lethalDamageType == EDamageType.GrenadeFragment || lethalDamageType == EDamageType.Explosion))
                End = "<b><color=#" + 
                      ColorUtility.ToHtmlStringRGB(victimColor) + 
                      ">" + 
                      victimRoleName + 
                      "</b></color><color=#" + 
                      ColorUtility.ToHtmlStringRGB(AHitmarkerPlugin.ThrowWeaponColor.Value) + 
                      "> " + 
                      victimNickname;
            else
                End = "<b><color=#" + 
    ...
    static async void LoadSprite(string path)
    {
        LoadedSprites[Path.GetFileName(path)] = await RequestSprite(path);
    }
    static async void LoadRanks(string path)
    {
        LoadedRanks[Path.GetFileName(path)] = await RequestSprite(path);
    }
    static async Task<Sprite> RequestSprite(string path)
    {
