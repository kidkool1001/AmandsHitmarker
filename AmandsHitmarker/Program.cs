...
public class AmandsMenuUIPatch : ModulePatch
{
    ...
    {
        if (AmandsHitmarkerClass.ActiveUIScreen == __instance.transform.GetChild(0).gameObject) 
            return;
...
public class AmandsBattleUIScreenPatch : ModulePatch
{
    ...
    {
        if (AmandsHitmarkerClass.ActiveUIScreen == __instance.gameObject) 
            return;
...
public class AmandsDamagePatch : ModulePatch
{
    ...
    private static void PatchPostFix(ref Player __instance, DamageInfoStruct damageInfo, EBodyPart bodyPartType)
    {
        ...
        if (player != null)
        {
            IsYourPlayerAgresssor = AmandsHitmarkerClass.Player != null && 
                                    player == AmandsHitmarkerClass.Player;
        }
        else
        {
        ...
        if (IsYourPlayerAgresssor)
        {
            ...
            if (AmandsHitmarkerClass.damageNumberTextMeshPro == null ||
                (!AHitmarkerPlugin.EnableDamageNumber.Value || !(damageInfo.DidBodyDamage > 0.01f)) &&
...
public class AmandsArmorDamagePatch : ModulePatch
{
    ...
    {
        if (!(armorDamage > 0) ||
            !armorComponents.Contains(__instance)) 
            return;
...
public class AmandsKillPatch : ModulePatch
{
    ...
    {
        if (AmandsHitmarkerClass.Player != null && 
            aggressor == AmandsHitmarkerClass.Player && 
            __instance != AmandsHitmarkerClass.Player && 
        ...
        else if (!aggressor.IsYourPlayer && 
                 FikaBackendUtils.IsServer)
        {
            if (aggressor is FikaPlayer coopAggressor && 
                coopAggressor != null)
                AmandsHitmarkerFikaBridge.SendKill(aggressor, __instance, damageInfo, bodyPart, lethalDamageType);
        }
        else if (AmandsHitmarkerClass.Player != null && 
                 aggressor == AmandsHitmarkerClass.Player && 
                 __instance != AmandsHitmarkerClass.Player && 
        ...
        if (AmandsHitmarkerClass.Player != null && 
            AmandsHitmarkerClass.Player != aggressor && 
            Vector3.Distance(AmandsHitmarkerClass.Player.Position, __instance.Position) > AHitmarkerPlugin.RaidKillDistance.Value) 
            return;
...
public class AmandsRaidAudioPatch : ModulePatch
{
    protected override MethodBase GetTargetMethod()
    {
        ...
        return typeof(BaseGameController).GetProperty("RaidStarted", BindingFlags.Instance | BindingFlags.Public)!.GetSetMethod(true);
    }
